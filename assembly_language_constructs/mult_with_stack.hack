...
(ENDLoop)
    ...
    // This is assuming that there is no overflow in sp
    @sp
    M=M+1  // where sp should be pointing to post operation also
    A=M
    A=M
...
(MFEnd) 
    ...
    D=A
    @sp
    A=M  // curr address points to address value stored in sp
    M=D
    @sp
    M=M-1 // decrement by 1
